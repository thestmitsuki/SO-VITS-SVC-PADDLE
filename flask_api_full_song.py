...
def wav2wav():
    ...
    for slice_tag, data in audio_data:
        ...
        if slice_tag:
            ...
            data = np.concatenate([np.zeros([pad_len]), data, np.zeros([
                pad_len])])
            raw_path = io.BytesIO()
    ...
    soundfile.write(out_wav_path, audio, svc_model.target_sample, format=
        wav_format)
    out_wav_path.seek(0)
    return send_file(out_wav_path, download_name=f'temp.{wav_format}',
        as_attachment=True)
